...
class actEval(nodeActEval,loadModel, FSM):
    ...
    def check_complete(self, is_complete):
        if(self.task_idx==0):
            self.state_change_m.update_complete(is_complete,2)
    ...
    def eval_bc(self):
        ...
        # print(f"image : {t2-t1}")
            query_frequency = 25
            time.sleep(0.015)
            if self.t % query_frequency == 0:
                if(not self.data_flag.camera_flag ):
                
                    print("camera not finish")
                    return
                time.sleep(0.1)
                self.all_actions, state_detect = self.policy(qpos, self.curr_image)

                state_detect = state_detect.cpu().numpy()
                self.check_complete(state_detect)
            raw_action = self.all_actions[:, self.t % query_frequency]
            ...
            if IS_REAL:
                self.lcm_unit.send_to_robot(np.array(target_qpos), waist_val)
