...
public final class doll_motor_debug_data_struct implements lcm.lcm.LCMEncodable
{
    ...
    public lcmtypes.doll_motor_debug_data_struct copy()
    {
        ...
        if (this.n_dof > 0)
            System.arraycopy(this.motor_pos_now, 0, outobj.motor_pos_now, 0, (int) this.n_dof); 
        outobj.motor_vel_now = new float[(int) n_dof];
        if (this.n_dof > 0)
            System.arraycopy(this.motor_vel_now, 0, outobj.motor_vel_now, 0, (int) this.n_dof); 
        outobj.motor_current_now = new float[(int) n_dof];
        if (this.n_dof > 0)
            System.arraycopy(this.motor_current_now, 0, outobj.motor_current_now, 0, (int) this.n_dof); 
        outobj.joint_pos_now = new float[(int) n_dof];
        if (this.n_dof > 0)
            System.arraycopy(this.joint_pos_now, 0, outobj.joint_pos_now, 0, (int) this.n_dof); 
        outobj.joint_vel_now = new float[(int) n_dof];
        if (this.n_dof > 0)
            System.arraycopy(this.joint_vel_now, 0, outobj.joint_vel_now, 0, (int) this.n_dof); 
        outobj.joint_current_now = new float[(int) n_dof];
        if (this.n_dof > 0)
            System.arraycopy(this.joint_current_now, 0, outobj.joint_current_now, 0, (int) this.n_dof); 
        return outobj;
    }
