...
class loadModel():
    ...
    def init_model(self, task_name):
        ...
        if self.vq:
            ...
            latent_model_ckpt_path = os.path.join(config["ckpt_dir"], 'latent_model_last.ckpt')
            self.latent_model.deserialize(torch.load(latent_model_ckpt_path,  map_location='cuda:0'))
