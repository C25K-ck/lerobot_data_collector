...
class lerobotAddscreenData():
    def __init__(self, repo_id,robot_type="magic_p6"):
        ...
        self.fps = 30
        self.target_dataset = create_empty_dataset(repo_id+"_screen", robot_type, mode="video", dataset_config=DEFAULT_DATASET_CONFIG)
    ...
    def update_frame(self):
        ...
        for i in range(len(self.source_dataset)):
            ...
            head_mask = generate_green_screen_mask_image_wrist(head_img)
            mask_pic1 = load_mask_image()
            head_img_output = add_random_lighting(merge_image(head_img,mask_pic1, head_mask))
            head_img_output = np.transpose(head_img_output, (2, 0, 1))

#                 head_screen_list.append(add_random_lighting(merge_image(head_rgb,mask_pic1, head_mask)))
            ...
            right_hand_mask = generate_green_screen_mask_image_wrist(right_hand_img)
            mask_pic2 = load_mask_image()

            right_hand_img_output = add_random_lighting(merge_image(right_hand_img,mask_pic2, right_hand_mask))
            right_hand_img_output = np.transpose(right_hand_img_output, (2, 0, 1))

            frame[f"observation.images.{"headf"}"] = torch.from_numpy(head_img_output)

            frame[f"observation.images.{"right_hand"}"] = torch.from_numpy(right_hand_img_output)
...
if __name__ == "__main__":
    s = lerobotAddscreenData(repo_id="t009/task_1_t009_0613")
