...
def main() -> None:
    ...
    queue_name = os.getenv("IMAGE_SAVE_QUEUE", "image_saver_left")
    save_dir = os.getenv("SAVE_DIR", "/tmp/realsense_left_images")
    ...
    def _callback(ch, method, properties, body):
        try:
            ...
            if log_detail:
                print(f"[worker-left] recv frame={frame_index}", flush=True)
            ...
            if log_detail:
                print(f"[worker-left] got redis bytes frame={frame_index}", flush=True)
            ...
            if log_detail:
                print(f"[worker-left] wrote rgb frame={frame_index}", flush=True)
            t_w2_0 = time.perf_counter()
            with open(depth_path, "wb") as f:
                f.write(depth_bytes)
            t_w2_1 = time.perf_counter()
            if log_detail:
                print(f"[worker-left] wrote depth frame={frame_index}", flush=True)
            if not (ok1 and ok2):
            ...
            if log_detail:
                print(f"[worker-left] acked frame={frame_index}", flush=True)
            ...
            if log_detail or (frame_index % log_every_n == 0):
                if log_detail:
                    print(
                        f"[worker-left] frame={frame_index} q_ms={q_latency_ms:.1f} "
    ...
    while not stop_flag["stop"]:
        ...
        if log_heartbeat and (time.time() - last_beat["t"]) >= 5.0:
            print(f"[worker-left] heartbeat saved={saved_count['n']} queue={queue_name}", flush=True)
            last_beat["t"] = time.time()
