...
class lcmUnit(LCM):
    def __init__(self) -> None:
        ...
        self.subscribe(self.state_estimator_topic, self.state_estimator_listener_cb)
        self.subscribe(self.leg_control_data_topic, self.leg_control_data_listener_cb)
        self.subscribe(self.leg_control_command_topic, self.leg_control_command_listener_cb)
    ...
    def upper_body_data_listener_cb(self, channel, data):
        try:
            ...
            self.current_robot_state.torque = np.array(msg.curTorqueVec)
            self.current_robot_state.current = np.array(msg.curCurrentVec)
            self.update_once_arm = True
        except Exception as e:
            print(f"upper_body_data_listener_cb err: {e}")
            
    def state_estimator_listener_cb(self, channel, data):
        try:
            msg = state_estimator_lcmt.decode(data)
            self.current_robot_state.body_imu_rpy = np.array(msg.rpy)
            self.current_robot_state.body_imu_rpy[2] = 0
            self.update_estimator_once = True
        except Exception as e:
            print(f"upper_body_data_listener_cb err: {e}")

    def leg_control_data_listener_cb(self, channel, data):
        try:
            msg = leg_control_data_lcmt.decode(data)
            self.current_robot_leg_state.state_q = np.array(msg.q)
            self.current_robot_leg_state.state_qd = np.array(msg.qd)
            self.current_robot_leg_state.state_tau = np.array(msg.tau_est)
            self.update_leg_state_once = True
        except Exception as e:
            print(f"upper_body_data_listener_cb err: {e}")

    def leg_control_command_listener_cb(self, channel, data):
        try:
            msg = leg_control_command_lcmt.decode(data)
            self.current_robot_leg_state.cmd_q = np.array(msg.q_des)
            self.current_robot_leg_state.cmd_qd = np.array(msg.qd_des)
            self.current_robot_leg_state.cmd_tau = np.array(msg.tau_ff)
    ...
    def load_upper_body_cmd_package(self, robot_30dof_solution):
        ...
        if self.gripper_selection[0]:
            if self.init_gripper_cnt < 19:
                ...
                upper_body_cmd_msg.jointPosVec[14] = 300
                ...
                upper_body_cmd_msg.control_mode[14:20] = (20 * np.ones(6).astype(int)).tolist()
                upper_body_cmd_msg.jointSpeedVec[14] = 300
        else :
        ...
        if self.gripper_selection[1]:
            if self.init_gripper_cnt < 19:
                ...
                upper_body_cmd_msg.jointSpeedVec[20] = 300
            elif self.init_gripper_cnt < 39 and self.init_gripper_cnt >= 19:
                upper_body_cmd_msg.control_mode[20:26] = (30 * np.ones(6).astype(int)).tolist()
                upper_body_cmd_msg.jointPosVec[20] = 300
            elif self.init_gripper_cnt < 59 and self.init_gripper_cnt >= 39:
                upper_body_cmd_msg.control_mode[20:26] = (201 * np.ones(6).astype(int)).tolist()
                upper_body_cmd_msg.jointSpeedVec[20] = 300
            else:
        ...
        if self.init_gripper_cnt > 50:
            self.init_gripper_cnt = 50
