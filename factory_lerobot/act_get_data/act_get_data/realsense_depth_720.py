...
from conf import REALSENSE_HEAD_SN
import ctypes
...
# 线程亲和性默认列表（可按需修改）
RS_HEAD_READ_CORES_DEFAULT = [4,5,6,7]
RS_HEAD_WRITE_CORES_DEFAULT = [12,13,14,15]
...
def save_images_async(color_image, depth_image, color_path, depth_path, timestamp_ns, timestamp_file):
    ...
    try:
        ...
        print(f"RealSense Head保存失败: {e}")

class SaveWorker:
    """固定工作线程+有界队列的保存器"""
    def __init__(self, maxsize=8, num_workers=1, name="Head"):
    ...
    def _worker_loop(self):
        while self.is_running:
            # 绑定保存线程亲和核
            _set_current_thread_affinity(RS_HEAD_WRITE_CORES_DEFAULT)
            try:
...
class RealsenseHeadRosController(Node):
    """RealSense Head相机ROS2控制节点"""
    def __init__(self):
        super().__init__('realsense_head_controller')
        ...
        self.get_logger().info('RealSense Head ROS2控制节点已启动')
    
    def camera_control_callback(self, msg):
        """处理相机控制指令"""
        command = msg.data
        self.get_logger().info(f'RealSense Head收到指令: {command}')
    ...
    def start_collection(self, session_id, start_timestamp=None, target_frames=None):
        ...
        #     self.get_logger().warning("RealSense Head已在采集中")
        #     return
        ...
        if start_timestamp:
            ...
            self.get_logger().info(f"RealSense Head准备采集，会话: {session_id}")
            self.get_logger().info(f"等待{delay:.1f}秒后开始，目标帧数: {target_frames}")
        else:
            self.get_logger().info(f"RealSense Head开始采集，会话: {session_id}")
    
    def stop_collection(self):
        """停止采集"""
        if not self.is_collecting:
            self.get_logger().warning("RealSense Head未在采集中")
            return
        
        self.is_collecting = False
        self.should_stop = True
        self.get_logger().info("RealSense Head停止采集")

def main():
    print("启动RealSense Head ROS2相机节点...")
    
    # 初始化ROS2
    rclpy.init()
    
    # 创建ROS2控制节点
    ros_controller = RealsenseHeadRosController()
    ...
    target_sn = REALSENSE_HEAD_SN
    
    # CPU亲和性设置
    current_process = psutil.Process(os.getpid())
    # 进程级亲和性为读/写核的并集
    _all_cores = sorted(set(RS_HEAD_READ_CORES_DEFAULT + RS_HEAD_WRITE_CORES_DEFAULT))
    if _all_cores:
        current_process.cpu_affinity(_all_cores)
    
    pipeline = None
    # 可通过环境变量 RS_HEAD_SAVE_QUEUE 配置保存队列大小
    _qsz_env = os.getenv('RS_HEAD_SAVE_QUEUE')
    try:
        _qsz = int(_qsz_env) if _qsz_env is not None else 128
    except Exception:
        _qsz = 128
    save_worker = SaveWorker(maxsize=_qsz, num_workers=1, name="Head")
    
    try:
        # 绑定主线程到读取核（采集/显示所在线程）
        _set_current_thread_affinity(RS_HEAD_READ_CORES_DEFAULT)

        # === RealSense相机初始化 ===
        print("初始化RealSense Head相机...")
        ...
        for dev in devices:
            if target_sn == dev.get_info(rs.camera_info.serial_number):
                device_found = True
                print(f"找到RealSense Head设备: SN={target_sn}")
                break
        
        if not device_found:
            print(f"未找到RealSense Head设备: SN={target_sn}")
            return
        ...
        print(f"RealSense Head连接成功: SN={actual_sn}")
        ...
        start_time_30 = None
        # 创建预览窗口
        window_name = "RealSense_Head_Preview"
        ...
        while rclpy.ok():
            ...
            if frames:
                try:
                    ...
                    if color_frame:
                        ...
                        # 显示预览画面
                        # display_img = cv2.resize(color_image, (640, 360))
            ...
            if key == ord('q'):
                print("用户按下q键，退出RealSense Head节点")
                break
            ...
            if ros_controller.is_collecting and ros_controller.current_session != last_session:
                ...
                rgb_dir = images_folder / "head_f_rgb"
                depth_dir = images_folder / "head_f_depth"
                ...
                # 创建时间戳文件 - 放在对应的图像文件夹里
                timestamp_file = rgb_dir / "head_f_timestamps.txt"
                # 清空之前的时间戳文件
                if timestamp_file.exists():
                    timestamp_file.unlink()
                
                frame_count = 0
                print(f"RealSense Head使用episode目录: {episode_folder}")
                print(f"RealSense Head时间戳文件: {timestamp_file}")
            ...
            if ros_controller.is_collecting and not ros_controller.should_stop and frames:
                try:
                    ...
                    if ros_controller.start_timestamp and current_time < ros_controller.start_timestamp:
                        ...
                        if frame_count == 0:  # 只在第一次显示等待信息
                            print(f"RealSense Head等待{remaining_wait:.1f}秒后开始采集...")
                    ...
                    if frame_count == 0:
                        ...
                        print(f"RealSense Head开始采集，目标帧数: {ros_controller.target_frames}")
                    
                    # 检查是否达到目标帧数
                    if ros_controller.target_frames > 0 and frame_count >= ros_controller.target_frames:
                        print(f"RealSense Head达到目标帧数{ros_controller.target_frames}，自动停止")
                    ...
                    if ros_controller.target_frames == 0 and ros_controller.collection_start_time:
                        if time.time() - ros_controller.collection_start_time >= duration:
                            print(f"RealSense Head采集时间到达{duration}秒，自动停止")
                            ros_controller.stop_collection()
                            continue
                    ...
                    if frame_count % 30 == 0 and ros_controller.collection_start_time:
                        ...
                        if ros_controller.target_frames > 0:
                            ...
                            print(f"RealSense Head: {frame_count}/{ros_controller.target_frames}帧 ({progress:.1f}%), {fps:.1f}fps, 剩余{remaining_frames}帧")
                        else:
                            remaining = duration - elapsed
                            print(f"RealSense Head: {frame_count}帧, {fps:.1f}fps, 剩余{remaining:.1f}s")
                
                except Exception as e:
                    print(f"RealSense Head采集错误: {e}")
                    continue
                ...
                print(f"RealSense Head采集完成: {frame_count}帧，用时{elapsed:.1f}秒")
                print(f"RealSense Head时间戳已保存到: {timestamp_file}")
                ros_controller.should_stop = False
                frame_count = 0

        
    except Exception as e:
        print(f"RealSense Head初始化失败: {e}")
        return
        ...
        print("RealSense Head ROS2相机节点停止")
