...
from conf import REALSENSE_RIGHT_SN
import ctypes
...
# 线程亲和性默认列表（可按需修改）
RS_RIGHT_READ_CORES_DEFAULT = [0,1,2,3,12]
RS_RIGHT_WRITE_CORES_DEFAULT = [13,14,15]
...
# 允许通过环境变量覆盖线程亲和核
RS_RIGHT_READ_CORES = _parse_cores_env('RS_RIGHT_READ_CORES', RS_RIGHT_READ_CORES_DEFAULT)
RS_RIGHT_WRITE_CORES = _parse_cores_env('RS_RIGHT_WRITE_CORES', RS_RIGHT_WRITE_CORES_DEFAULT)
...
def save_images_async(color_image, depth_image, color_path, depth_path, timestamp_ns, timestamp_file):
    ...
    try:
        ...
        print(f"RealSense Right保存失败: {e}")

class SaveWorker:
    """固定工作线程+有界队列的保存器"""
    def __init__(self, maxsize=8, num_workers=1, name="Right"):
    ...
    def _worker_loop(self):
        ...
        _set_current_thread_affinity(RS_RIGHT_WRITE_CORES)
        while self.is_running:
            ...
            try:
                # 将frame对象在保存线程中转换为ndarray，减少主线程拷贝
...
class RealsenseRightRosController(Node):
    """RealSense Right相机ROS2控制节点"""
    def __init__(self):
        super().__init__('realsense_right_controller')
        ...
        self.subscription = self.create_subscription(
            String,
            '/realsense_right_control',
            self.camera_control_callback,
            10
        )
        
        self.get_logger().info('RealSense Right ROS2控制节点已启动')
    
    def camera_control_callback(self, msg):
        """处理相机控制指令"""
        command = msg.data
        self.get_logger().info(f'RealSense Right收到指令: {command}')
    ...
    def start_collection(self, session_id, start_timestamp=None, target_frames=None):
        ...
        #     self.get_logger().warning("RealSense Right已在采集中")
        #     return
        ...
        if start_timestamp:
            ...
            self.get_logger().info(f"RealSense Right准备采集，会话: {session_id}")
            self.get_logger().info(f"等待{delay:.1f}秒后开始，目标帧数: {target_frames}")
        else:
            self.get_logger().info(f"RealSense Right开始采集，会话: {session_id}")
    
    def stop_collection(self):
        """停止采集"""
        if not self.is_collecting:
            self.get_logger().warning("RealSense Right未在采集中")
            return
        
        self.is_collecting = False
        self.should_stop = True
        self.get_logger().info("RealSense Right停止采集")

def main():
    print("启动RealSense Right ROS2相机节点...")
    
    # 初始化ROS2
    rclpy.init()
    
    # 创建ROS2控制节点
    ros_controller = RealsenseRightRosController()
    
    # 输出配置（移除兜底超时）
    
    # RealSense Right SN
    target_sn = REALSENSE_RIGHT_SN
    
    # 去除进程级CPU亲和性，仅保留线程级绑定
    
    pipeline = None
    # 可通过环境变量 RS_RIGHT_SAVE_QUEUE 配置保存队列大小
    _qsz_env = os.getenv('RS_RIGHT_SAVE_QUEUE')
    try:
        _qsz = int(_qsz_env) if _qsz_env is not None else 512
    except Exception:
        _qsz = 512
    save_worker = SaveWorker(maxsize=_qsz, num_workers=1, name="Right")
    try:
        cv2.setNumThreads(1)
    except Exception:
        pass
    ...
    try:
        # 绑定主线程到读取核（采集/显示所在线程）
        _set_current_thread_affinity(RS_RIGHT_READ_CORES)

        # === RealSense相机初始化 ===
        print("初始化RealSense Right相机...")
        ...
        for dev in devices:
            if target_sn == dev.get_info(rs.camera_info.serial_number):
                device_found = True
                print(f"找到RealSense Right设备: SN={target_sn}")
                break
        
        if not device_found:
            print(f"未找到RealSense Right设备: SN={target_sn}")
            return
        ...
        print(f"RealSense Right连接成功: SN={actual_sn}")
        ...
        # 创建预览窗口（始终显示）
        window_name = "RealSense_Right_Preview"
        ...
        while rclpy.ok():
            ...
            if frame_count_30 >= 30:
                ...
                fps = 30 / elapsed_time
                print(f"R获取30张图片耗时: {elapsed_time:.3f}秒, 平均FPS: {fps:.2f}")
                start_time_30 = time.time()
                frame_count_30 = 0
            # 检查退出键
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'):
                print("用户按下q键，退出RealSense Right节点")    
                break
            ...
            if ros_controller.is_collecting and ros_controller.current_session != last_session:
                ...
                rgb_dir = images_folder / "hand_r_rgb"
                depth_dir = images_folder / "hand_r_depth"
                ...
                frame_count = 0  # 🔥 重置帧计数
                print(f"RealSense Right使用episode目录: {episode_folder}")
                print(f"RealSense Right时间戳文件: {timestamp_file}")
                print(f"RealSense Right已清空并重新开始采集")
                ...
                try:
                    ...
                    if ros_controller.start_timestamp and current_time < ros_controller.start_timestamp:
                        ...
                        if frame_count == 0:  # 只在第一次显示等待信息
                            print(f"RealSense Right等待{remaining_wait:.1f}秒后开始采集...")
                    ...
                    if frame_count == 0:
                        ...
                        print(f"RealSense Right开始采集，目标帧数: {ros_controller.target_frames}")
                    
                    # 检查是否达到目标帧数
                    if ros_controller.target_frames > 0 and frame_count >= ros_controller.target_frames:
                        print(f"RealSense Right达到目标帧数{ros_controller.target_frames}，自动停止")
            ...
            # 无兜底超时停止逻辑
                    ...
                    if frame_count % 30 == 0 and ros_controller.collection_start_time:
                        ...
                        if ros_controller.target_frames > 0:
                            ...
                            print(f"RealSense Right: {frame_count}/{ros_controller.target_frames}帧 ({progress:.1f}%), {fps:.1f}fps, 剩余{remaining_frames}帧")
                        else:
                            print(f"RealSense Right: {frame_count}帧, {fps:.1f}fps")
                
                except Exception as e:
                    print(f"RealSense Right采集错误: {e}")
                    continue
                ...
                print(f"RealSense Right采集完成: {frame_count}帧，用时{elapsed:.1f}秒")
                print(f"RealSense Right时间戳已保存到: {timestamp_file}")
                ros_controller.should_stop = False
                frame_count = 0

        
    except Exception as e:
        print(f"RealSense Right初始化失败: {e}")
        return
        ...
        print("RealSense Right ROS2相机节点停止")
