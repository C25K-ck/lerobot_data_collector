...
LeRobot 数据集结构中（images/hand_l_xxx）。
...
try:
    # 兼容脚本直跑与包内导入两种方式
    from conf import REALSENSE_LEFT_SN  # type: ignore[import]
    ...
    def _loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.warning("RealSense 左手保存失败: %s", exc)
            finally:
                del color_image
                del depth_image
                self.queue.task_done()
...
class RealSenseLeftCameraManager:
    """负责 RealSense 左手相机的预览与录制。"""
    ...
    def __init__(
        ...
        self.serial_number = serial_number or REALSENSE_LEFT_SN
    ...
    def start(self) -> bool:
        ...
        logger.info("RealSense 左手预览线程已启动")
        return True
        ...
        while self._running and self.pipeline:
            ...
            if self.align:
                try:
                    ...
                    logger.debug("RealSense 左手对齐失败，重试: %s", exc)
                    continue
            ...
            if self._recording and self._episode_paths:
                ...
                if (
                    ...
                    logger.info(
                        "RealSense 左手达到目标帧数 %d（相机侧），停止该路写盘以确保帧数对齐",
                        self._record_target_frames,
                    )
                    self._recording = False
                    self._record_target_frames = None
    ...
    def _prepare_episode_dirs(self, dataset_root: Path) -> EpisodePaths:
        ...
        rgb_dir = images_dir / "hand_l_rgb"
        depth_dir = images_dir / "hand_l_depth"
...
def main() -> None:  # pragma: no cover - 用于手动调试
    ...
    manager = RealSenseLeftCameraManager()
    if not manager.start():
        print(f"RealSense 左手不可用：{manager.last_error}")
        return

    repo_id = f"realsense_left_demo_{int(time.time())}"
