...
class controlHandle(baseMocapHandle):
    ...
    def odd_trans(self, odd,is_left):
        ...
        if(is_left):
            odd_trans[:3,:3] = eul_to_rot(np.array((3.1415926,0,1.5708)))
        else:
            odd_trans[:3,:3] = eul_to_rot(np.array((0, 3.1415926, 1.5708)))
...
class mocapDataManage():
    def __init__(self):
        self.left_arm_ctrl = controlHandle(True)
        self.right_arm_ctrl = controlHandle(False)

        self.chest_ctrl = controlHandle(False)
        self.hmd_ctrl = controlHandle(False)
    ...
    def parse_data(self, data_dict):
        if (self.pre_count > 0):
            ...
            self.chest_ctrl.prepare_base(self.pos_eul_trans(data_dict['hmd']["p_e"]))
            self.base_pos_eul = self.chest_ctrl.init_pos_eul


            self.left_arm_ctrl.prepare(self.pos_eul_trans(data_dict["l_h"]["p_e"]), self.base_pos_eul)
            self.right_arm_ctrl.prepare(self.pos_eul_trans(data_dict["r_h"]["p_e"]), self.base_pos_eul)


            self.pre_count -= 1

        else:
            self.chest_ctrl.update_base(self.pos_eul_trans(data_dict['hmd']["p_e"]))


            self.left_arm_ctrl.update(np.array(data_dict["l_h"]["p_e"]), self.chest_ctrl.pos_eul)
            self.right_arm_ctrl.update(self.pos_eul_trans(data_dict["r_h"]["p_e"]), self.chest_ctrl.pos_eul)
    ...
    def update_button(self, data_dict):
        self.left_arm_ctrl.update_btn(data_dict["l_h"])
        self.right_arm_ctrl.update_btn(data_dict["r_h"])
